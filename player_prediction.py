...
.st-emotion-cache-1g6xema { /* Metric labels */
...
def get_team_roster(team_id, season):
    roster = commonteamroster.CommonTeamRoster(team_id=team_id, season=season, headers=CUSTOM_HEADERS, timeout=60)
...
def plot_performance_chart(df, player_name, stat, prop_line):
    ...
    fig.add_hline(y=prop_line, line_dash="dash", line_color="#FF4B2B",
                  annotation_text=f"Prop Line: {prop_line}", annotation_position="bottom right")
    fig.update_layout(
        template="plotly_dark",
        title={'text': f'{player_name} - {stat} Over Time', 'y': 0.9, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top'},
        font=dict(family="Roboto Condensed, sans-serif")
    )
    st.plotly_chart(fig, use_container_width=True)


# <<< RESTORED >>> The function to format the recent games DataFrame for display.
def get_recent_games_df(df):
    if df.empty: return pd.DataFrame()
    display_cols = {
        'GAME_DATE': 'Date', 'MIN': 'Min', 'PTS': 'Pts', 'REB': 'Reb', 'AST': 'Ast',
        'STL': 'Stl', 'BLK': 'Blk', 'TOV': 'TO', 'FG_PCT': 'FG%', 'FG3_PCT': '3P%', 'FT_PCT': 'FT%'
    }
...
def main():
    ...
    if selected_team_id:
        ...
        roster_df = get_team_roster(selected_team_id, season_select)
    ...
    game_log_df = game_log_df.sort_values('GAME_DATE', ascending=False)  # Sort descending to easily get recent games

    col1, col2 = st.columns([1, 4])
    with col1:
        st.image(get_player_headshot_url(selected_player_id))
    with col2:
    ...
    # --- Charting and Data Section ---
    c1, c2 = st.columns((2, 1))  # Re-establish the two-column layout
    with c1:
        ...
        # We pass the already-sliced and sorted-descending df, then reverse for plotting
        plot_performance_chart(recent_games_df.sort_values('GAME_DATE', ascending=True), full_name, selected_stat,
                               prop_line)

    # <<< RESTORED >>> The Recent Game Logs table is back in the second column.
    with c2:
